...
int panel_set_dsu(struct panel_device *panel, struct dsu_info *dsu)
{
	...
	panel_info("mres_mode : xres : %d, yres : %d\n", xres, yres);
...
static int panel_parse_lcd_info(struct panel_device *panel)
{
...
#ifdef CONFIG_SUPPORT_DSU
	int mres_number, i;
	unsigned int dsu_res[MAX_RES_NUMBER];
	struct lcd_mres_info *dt_lcd_mres = &panel_info->dt_lcd_mres;
#endif
	u32 hdr_num = 0, j;
	u32 hdr_type[HDR_CAPA_NUM] = {0, };
	struct lcd_hdr_info *dt_lcd_hdr = &panel_info->dt_lcd_hdr;
...
#ifdef CONFIG_SUPPORT_DSU
	of_property_read_u32(node, "mres_en", &dt_lcd_mres->mres_en);
	panel_info("PANEL:INFO:%s:mres_en : %d\n", __func__, dt_lcd_mres->mres_en);

	if (dt_lcd_mres->mres_en) {
		of_property_read_u32(node, "mres_number", &mres_number);
		panel_info("PANEL:INFO:%s:mres_number : %d\n", __func__, mres_number);
		if (mres_number > MAX_RES_NUMBER) {
			panel_err("PANEL:ERR:%s:Exceed dsu res number : %d\n",
				__func__, mres_number);
			mres_number = MAX_RES_NUMBER;
		}
		dt_lcd_mres->mres_number = mres_number;

		of_property_read_u32_array(node, "mres_width", dsu_res, mres_number);
		for (i = 0; i < mres_number; i++) {
			dt_lcd_mres->res_info[i].width = dsu_res[i];
			panel_info("PANEL:INFO:%s:width[%d]:%d\n", __func__, i, dsu_res[i]);
		}
		of_property_read_u32_array(node, "mres_height", dsu_res, mres_number);
		for (i = 0; i < mres_number; i++) {
			dt_lcd_mres->res_info[i].height = dsu_res[i];
			panel_info("PANEL:INFO:%s:height[%d]:%d\n", __func__, i, dsu_res[i]);
		}
		of_property_read_u32_array(node, "mres_dsc_en", dsu_res, mres_number);
		for (i = 0; i < mres_number; i++) {
			dt_lcd_mres->res_info[i].dsc_en = dsu_res[i];
			panel_info("PANEL:INFO:%s:dsc_en[%d]:%d\n", __func__, i, dsu_res[i]);
		}

		of_property_read_u32_array(node, "mres_dsc_width", dsu_res, mres_number);
		for (i = 0; i < mres_number; i++) {
			dt_lcd_mres->res_info[i].dsc_width = dsu_res[i];
			panel_info("PANEL:INFO:%s:dsc_width[%d]:%d\n", __func__, i, dsu_res[i]);
		}
		of_property_read_u32_array(node, "mres_dsc_height", dsu_res, mres_number);
		for (i = 0; i < mres_number; i++) {
			dt_lcd_mres->res_info[i].dsc_height= dsu_res[i];
	...
	dt_lcd_hdr->hdr_num = hdr_num;
	panel_info("PANEL:INFO:%s:hdr_num(%d)\n", __func__, hdr_num);

	if (hdr_num != 0) {
		of_property_read_u32_array(node, "hdr_type", hdr_type, hdr_num);
		for (j = 0; j < hdr_num; j++) {
			dt_lcd_hdr->hdr_type[j] = hdr_type[j];
			panel_info("PANEL:INFO:%s:hdr_type[%d] = %d\n",
				__func__, j, dt_lcd_hdr->hdr_type[j]);
		}

		of_property_read_u32(node, "hdr_max_luma", &dt_lcd_hdr->hdr_max_luma);
		of_property_read_u32(node, "hdr_max_avg_luma", &dt_lcd_hdr->hdr_max_avg_luma);
		of_property_read_u32(node, "hdr_min_luma", &dt_lcd_hdr->hdr_min_luma);

		panel_info("PANEL:INFO:%s:hdr_max_luma(%d), hdr_max_avg_luma(%d), hdr_min_luma(%d)\n",
				__func__, dt_lcd_hdr->hdr_max_luma, dt_lcd_hdr->hdr_max_avg_luma,
				dt_lcd_hdr->hdr_min_luma);
