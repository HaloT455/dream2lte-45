...
static int dsu_check_info(struct decon_win_config *dsu_info, struct lcd_mres_info *dt_dsi_info) 
{
	int actual_mode;
	int width, height;
...
int dsu_win_config(struct decon_device *decon, struct decon_win_config *windata, struct decon_reg_data *regs)
{
	...
	struct lcd_mres_info *dt_lcd_mres = &decon->lcd_info->dt_lcd_mres;
	...
	if (unlikely(dt_lcd_mres->mres_en == 0)) {
	...
	ret = dsu_check_info(new_dsu, dt_lcd_mres);
	if (unlikely(ret != 0))
		return ret;
